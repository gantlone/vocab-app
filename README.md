...
- 每次練習自動混入「到期複習單字」（最多佔 60%），強化記憶曲線
- 已標記「熟悉」的單字不再出現於練習池，讓練習集中在尚未掌握的字

### 進度備份
- 右上角「**匯出**」：下載目前進度為 JSON 備份檔
- 右上角「**匯入**」：選擇備份檔還原進度（支援跨裝置手動搬移）
...
每個單字都有一個 **Level 0–5**，代表你對這個字的熟悉程度：

| Level | 意義 | 下次複習間隔 |
|-------|------|------------|
| 0 | 不熟，需要立刻再練 | 0 天（下次練習立即出現） |
| 1 | 看過一次 | 1 天後 |
| 2 | 普通 | 3 天後 |
| 3 | 還不錯 | 7 天後 |
| 4 | **熟悉**（不再主動出現） | 14 天後偶爾複習 |
| 5 | 非常熟練 | 30 天後偶爾複習 |

**練習流程：**

1. 進入練習時，系統先找出「**到期單字**」（`nextReview <= 現在時間`）
2. 到期單字最多佔當輪練習的 60%，其餘從該頁未熟悉的字隨機補足
3. 答對 → Level +1，下次間隔拉長；答錯 → Level -1，很快再出現
4. Level ≥ 4 的字標記為「熟悉」，不再佔練習名額，讓你專注練還不會的字

> **跨頁追蹤**：到期單字來自全部頁面，不限當前練習的頁碼。你在 page 3 答錯的字，在 page 7 的練習裡也可能出現（最多佔 60%），直到你答對讓 Level 升上去為止。

---

## 重置學習進度

點擊右上角「重置」按鈕，或手動清空 `src/data/memory.json`：
